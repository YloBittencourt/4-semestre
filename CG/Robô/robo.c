...
// Função DesenhaQuadrado conforme pedido
...
void Desenho(void) {
    ...
    // Braço Esquerdo (seguindo a hierarquia)
    //Ombro
    glPushMatrix();
    ...
    // Braço Direito (seguindo a hierarquia)
    glPushMatrix();
        glTranslatef(1.5, 2.0, 0.0);
...
'5' '6': Tronco
'A' 'S': Ombro Esquerdo
'D' 'F': Cotovelo Esquerdo
'G' 'H': Mão Esquerda
'U' 'I': Dedo1 Esquerdo
'O' 'P': Dedo2 Esquerdo
'Q' 'W': Ombro Direito
'E' 'R': Cotovelo Direito
'T' 'Y': Mão Direita
'Z' 'X': Dedo1 Direita
'C' 'V': Dedo2 Direita
'J' 'K': Perna Esquerda
